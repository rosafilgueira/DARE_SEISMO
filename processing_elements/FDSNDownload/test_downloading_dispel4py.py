...
def waveform_reader(data):
    filename = data
    st = read(filename)
    prov = {'location': "file://" + socket.gethostname() + "/" + filename, 'format': 'application/octet-stream',
            'metadata': {'prov:type': 'observed-waveform'}}
...
def stationxml_reader(data):
    ...
    scode = s.get('code')
    prov = {'location': "file://" + socket.gethostname() + "/" + filename, 'format': 'application/xml',
            'metadata': {'prov:type': 'stationxml', 'station': scode, 'network': ncode}}
